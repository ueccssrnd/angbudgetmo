...
BudgetMo = (function() {
  ...
  BudgetMo.prototype.ui = {
    ...
    build: function() {
      var arc, canvas, collatedArcs, collatedData, collatedDoughnut, collatedGroup, color, expanded, govArcs, govData, govDoughnut, govGroup, height, k, layout, line, radius, sectors, v, width;
      ...
      radius = 150;
      canvas = d3.select("#content").append("svg").style("width", width).style("height", height).style("position", "absolute").style("top", 100).style("left", '50%').style("margin-left", -1 * width / 2 + "px").style("transform", "translate(10, '-50%')");
      ...
      return govGroup.append("circle").classed("nav-collapse", true).classed("nav", true).attr("cx", 0).attr("cy", 0).attr("r", radius * 0.4 - 10).attr("fill", color(radius * 0.4 - 10));
    },
    lightbox: {
      show: function() {
